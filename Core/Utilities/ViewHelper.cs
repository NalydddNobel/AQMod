...
public static class ViewHelper {
    public const Single Z_VIEW = -20f;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void PunchCameraTo(Vector2 position, Single strength = 2f, Single vibrationsPerSecond = 4f, Int32 frames = 10) {
        ...
        Single intensity = 1f - distance / 150f / strength;
        if (intensity > 0f) {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void PunchCamera(Vector2 direction, Single strength = 2f, Single vibrationsPerSecond = 4f, Int32 frames = 10) {
        if (Main.netMode != NetmodeID.Server) {
            Main.instance.CameraModifiers.Add(new PunchCameraModifier(Vector2.Zero, direction, strength, vibrationsPerSecond, frames));
        }
    }

    public static Vector2 GetViewPoint(Vector2 origin, Single z) {
    ...
    public static Vector2 GetViewPoint(Vector2 origin, Single z, Vector2 viewPos) {
        return new Vector2(origin.X - (1f - (-Z_VIEW / (z - Z_VIEW))) * (origin.X - viewPos.X), origin.Y - (1f - (-Z_VIEW / (z - Z_VIEW))) * (origin.Y - viewPos.Y));
    }

    public static Single GetViewScale(Single originalScale, Single z) {

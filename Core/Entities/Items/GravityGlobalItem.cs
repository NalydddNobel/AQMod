...
public sealed class GravityGlobalItem : GlobalItem {
    ...
    public void SetNoGrav(Item item, byte duration) {
        var AequusRemake = item.GetGlobalItem<GravityGlobalItem>();
        if (AequusRemake.itemGravityCheck > Math.Max(duration - 15, 15)) {
            return;
        }
        CheckNoGravity(item);
        AequusRemake.itemGravityCheck = duration;
    }
    ...
    public override void OnSpawn(Item item, IEntitySource source) {
        if (source is EntitySource_Parent entitySource_Parent) {
            //if (parent is not NPC npc || !npc.GetGlobalNPC<AequusRemakeNPC>().noGravityDrops || Helper.FindFloor(parent.Center, 20) != -1) {
            //    return;
            //}

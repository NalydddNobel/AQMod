...
public class HamaYumiArrow : ModProjectile {
    ...
    public static readonly Color BloomColor = new Color(20, 2, 80, 10);
    ...
    public override bool PreDraw(ref Color lightColor) {
        ...
        DrawHelper.VertexStrip.PrepareStrip(Projectile.oldPos, Projectile.oldRot,
            (p) => HamaYumiArrow.BloomColor * 3 * (1f - p),
            (p) => 8f,
            Projectile.Size / 2f);
        ...
        Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, frame, FireColor * Projectile.Opacity * opacity, Projectile.rotation, origin, Projectile.scale, SpriteEffects.None, 0f);
        return false;
    }
}
...
public class HamaYumiExplosion : ModProjectile {
    ...
    public override Color? GetAlpha(Color lightColor) {
        return HamaYumiArrow.BloomColor with { A = 30 } * 5;
    }
    ...
    public override void AI() {
        if (Projectile.frame == 0 && Main.netMode != NetmodeID.Server) {
            ...
            foreach (var p in ModContent.GetInstance<LegacyBloomParticle>().NewMultiple(5)) {
                ...
                p.Color = HamaYumiArrow.FireColor;
                p.BloomColor = HamaYumiArrow.BloomColor * 0.2f;
                p.Scale = 1.25f;
                p.BloomScale = 0.3f;

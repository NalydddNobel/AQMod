...
public class StreamingBalloon : ModNPC, IBodyItemContainer {
    ...
    public int Stack { get => (int)NPC.ai[3]; set => NPC.ai[3] = value; }
    ...
    public override void SetStaticDefaults() {
        ...
        NPCSets.ImmuneToAllBuffs[Type] = true;
    }
    ...
    public override void HitEffect(NPC.HitInfo hit) {
        ...
        if (Main.netMode != NetmodeID.MultiplayerClient && (int)NPC.ai[0] > -1 && (int)NPC.ai[0] < Main.maxNPCs) {
            NPC spawnNPC = NPC.NewNPCDirect(NPC.GetSource_FromThis(), SlaveNPC.Center, SlaveNPC.type, SlaveNPC.whoAmI, SlaveNPC.ai[0], SlaveNPC.ai[1], SlaveNPC.ai[2], SlaveNPC.ai[3], NPC.target);
            spawnNPC.life = SlaveNPC.life;

            if (Main.netMode == NetmodeID.Server) {
                ExtendNPC.SyncNPC(spawnNPC);
            }
    ...
    public override void AI() {
        ...
        if ((int)NPC.ai[2] == 0) {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient) {
                if (NPC.HasValidTarget) {
                    ...
                    else if (Main.xMas && Main.rand.NextBool()) {
                    ...
                    if ((int)NPC.ai[2] == NPCID.BlueSlime || (int)NPC.ai[2] < 0) {
                        IBodyItemContainer.RollRules(NPC, Main.player[NPC.target]);
                    }

                    if (Main.netMode == NetmodeID.Server) {
                        ExtendNPC.SyncNPC(Main.npc[n]);
                    }
        ...
        if (distance > 1650f) {
            NPC.life = 0;
            if (Main.netMode != NetmodeID.Server) {
                SoundEngine.PlaySound(SoundID.Item111 with { Volume = 1.5f, Pitch = 0.9f }, NPC.Center);
            }
            NPC.HitEffect();
            NPC.active = false;
        }
        ...
        if (NPC.active) {
            var npc = SlaveNPC;
            npc.velocity = Vector2.UnitY * 0.01f;
        ...
        if (NPC.ai[1] > 0f) {
            // Item stack is not inherited.
            SlaveNPC.ai[1] = ItemId;
        }
    }
    ...
    public override void ModifyNPCLoot(NPCLoot npcLoot) {
        // Skyware Chest loot.
        IItemDropRule bodyRules = new OneFromOptionsBodyDropRule(10, 1,
        ...
        IItemDropRule finalRuleChains = bodyRules
            // Fledgling Wings.
            .OnFailedRoll(new CommonBodyDropRule(ItemID.CreativeWings, 100))
            // Giant Harpy Feather.
            .OnFailedRoll(new CommonBodyDropRule(ItemID.GiantHarpyFeather, 100))
            // Windy Day kites.
            .OnFailedRoll(new OneFromOptionsBodyDropRule(25, 1,
                ItemID.KiteBlue,
            ...
            // Furniture.
            .OnFailedRoll(new OneFromOptionsBodyDropRule(15, 1,
                ItemID.SkyMill,
                ItemID.SkywareChest,
        ...
        // Chain paper airplane rules onto the final rules.
        finalRuleChains.OnFailedRoll(new CommonBodyDropRule(ItemID.PaperAirplaneA, 4));
        finalRuleChains.OnFailedRoll(new CommonBodyDropRule(ItemID.PaperAirplaneB, 4));
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor) {
        if (NPC.IsABestiaryIconDummy) {
            _balloonFrame = (int)Main.GameUpdateCount / 90 % (FramesX - 1);
        }
        ...
        if (NPC.IsABestiaryIconDummy) {
            ...
            int index = (int)Main.GameUpdateCount / 30 % (spawnableEnemies.Count - 1);

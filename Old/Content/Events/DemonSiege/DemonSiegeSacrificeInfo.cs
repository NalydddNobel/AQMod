...
public class DemonSiegeSacrificeInfo {
    ...
    public int DetermineLength() {
        ...
        foreach (var i in Items) {
            if (GoreNestConversions.OriginalToConversion.TryGetValue(i.netID, out var value)) {
    ...
    public void InnerUpdate_OnEnd() {
        ...
        if (Main.netMode != NetmodeID.MultiplayerClient) {
            ...
            foreach (var i in Items) {
                GoreNestConversions.OriginalToConversion.TryGetValue(i.type, out var value);
    ...
    public void OnFail_PukeItems(bool clientOnly) {
        ...
        foreach (var i in Items) {
            if (GoreNestConversions.OriginalToConversion.TryGetValue(i.type, out var val) && val.OriginalItem == val.NewItem) {
                continue;
            }
            if (!clientOnly) {
    ...
    public void OnFail_EatItems(bool clientOnly) {
        string itemList = "";
        foreach (var i in Items) {
            if (GoreNestConversions.OriginalToConversion.TryGetValue(i.type, out var val) && val.OriginalItem != val.NewItem/* || val.OriginalItem == ModContent.ItemType<VoidRing>()*/) {
                continue;
            }
            if (itemList != "") {
                itemList += ", ";

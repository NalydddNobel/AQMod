    ...
    public class AQPlayer : ModPlayer
    {
        ...
        public override Texture2D GetMapBackgroundImage()
        {
            if (!player.ZoneCorrupt && !player.ZoneCrimson && !player.ZoneHoly && !player.ZoneDesert && !player.ZoneJungle)
            {
                if (player.position.Y < Main.worldSurface * 16f)
                {
                    ...
                    {
                        return SpriteUtils.Textures.Extras[ExtraID.GlimmerMapBackground];
                    }
                }
            }
            return null;
        ...
        public override void PostUpdateEquips()
        {
            ...
            if (arachnotron)
            {
                if (Main.myPlayer == player.whoAmI)
                {
                    int type = ModContent.ProjectileType<Projectiles.MechaSpiderLegs>();
                    if (player.ownedProjectileCounts[type] <= 0)
                    {
            ...
            if (omori)
            {
                ...
                int type = ModContent.ProjectileType<Projectiles.Friend>();
        ...
        public override void CatchFish(Item fishingRod, Item bait, int power, int liquidType, int poolSize, int worldLayer, int questFish, ref int caughtType, ref bool junk)
        {
            if (liquidType == Tile.Liquid_Water)
            {
                ...
                if (player.ZoneBeach && Main.rand.NextBool(10))
                {
                    if (Robster.StoryProgression == Robster.STORY_FIND_SNOBSTER && !player.HasItem(ModContent.ItemType<Snobster>()))
                    {
                        caughtType = ModContent.ItemType<Snobster>();
                    }
                    else if (Robster.StoryProgression == Robster.STORY_FIND_CRAB_CARD && !player.HasItem(ModContent.ItemType<CrabCard>()))
                    {
                        caughtType = ModContent.ItemType<CrabCard>();
                    }
                }
            }
        ...
        public override void DrawEffects(PlayerDrawInfo drawInfo, ref float r, ref float g, ref float b, ref float a, ref bool fullBright)
        {
            if (sparkling)
            {
                if (drawInfo.shadow == 0f)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        int d = Dust.NewDust(drawInfo.position - new Vector2(2f, 2f), player.width + 4, player.height + 4, ModContent.DustType<Dusts.UltimaDust>(), player.velocity.X * 0.4f, player.velocity.Y * 0.4f, 100, default(Color), Main.rand.NextFloat(0.45f, 1f));

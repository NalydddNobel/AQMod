...
namespace Aequus
{
    public class AequusWorld : ModSystem
    {
        ...
        private void ConstellationTest()
        {
            if (Main.star == null)
                return;
            ...
            int x = 0;
            Main.numStars = 500;
            if (Main.star.Length < text.Length)
            {
                Array.Resize(ref Main.star, text.Length);
            }
            int starIndex = 0;
            for (int textIndex = 0; textIndex < text.Length; textIndex++)
            {
                ...
                Star s = Main.star[starIndex];
                if (s == null)
                {
                    s = Main.star[starIndex] = new Star();
                    Star.SpawnStars(starIndex);
                    Main.NewText("null star");
                }
                if (s == null)
                {
                    Main.NewText("null star ID: "+ starIndex + ", " + textIndex);
                    break;
                }
                s.type = 0;
                ...
                if (c != '░')
                {
                    s.position = new Vector2(x * 5f + Main.screenWidth / 2f, y * 10f + Main.screenHeight / 2f);
                    starIndex++;
                }
                x++;
            }

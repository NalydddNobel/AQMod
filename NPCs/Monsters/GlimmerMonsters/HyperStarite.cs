...
namespace AQMod.NPCs.Monsters.GlimmerMonsters
{
    public class HyperStarite : ModNPC
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            var texture1 = LegacyTextureCache.Lights[LightTex.Spotlight66x66];
            ...
            if (npc.ai[0] == 2f)
            {
                var texture3 = LegacyTextureCache.Lights[LightTex.Spotlight33x24];
                float rotation;
                if (npc.ai[2] != 0f)
                {
            ...
            Main.spriteBatch.Draw(texture, new Vector2((int)(npc.position.X + offset.X - Main.screenPosition.X), (int)(npc.position.Y + offset.Y - Main.screenPosition.Y)), npc.frame, new Color(60, 60, 60, 0) * AQConfigClient.Instance.EffectIntensity, 0f, origin, npc.scale + 0.3f, SpriteEffects.None, 0f);
            return false;
        }
    }
}
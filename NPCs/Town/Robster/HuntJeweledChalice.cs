using AQMod.NPCs.Town.Robster.QuestItems;
...
namespace AQMod.NPCs.Town.Robster
{
    public class HuntJeweledChalice : RobsterHunt
    {
        ...
        public override int GetQuestItem() => ModContent.ItemType<JeweledChalice>();
                ...
                if (Main.npc[npc].active && Main.npc[npc].townNPC && !Main.npc[npc].homeless && Main.npc[npc].type != ModContent.NPCType<Robster>())
                {
                    ...
                    for (int k = checkRectangle.X; k < checkRectangle.X + checkRectangle.Width; k++)
                    {
                        for (int l = checkRectangle.Y; l < checkRectangle.Y + checkRectangle.Height; l++)
                        {
                            ...
                            {
                                WorldGen.PlaceTile(randomX, randomY, ModContent.TileType<QuestTiles.JeweledChalice>(), true, false, -1, 0);
                                if (Framing.GetTileSafely(randomX, randomY).type == ModContent.TileType<QuestTiles.JeweledChalice>())
                                {

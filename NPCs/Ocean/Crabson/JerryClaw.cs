...
{
    [AutoloadBossHead]
    public class JerryClaw : ModNPC, NPCIMethods.idecidetofallthroughplatforms
    {
        ...
        public override void SetDefaults()
        {
            ...
            npc.SetLiquidSpeed(1.5f, 0.5f, 0.25f);
        }
        ...
        public override void AI()
        {
            ...
            if (Main.npc[otherClaw].active)
            {
                ...
                if ((int)npc.ai[2] == PHASE_JUMPING_AND_SETTING_UP_ATTACKS_I_THINK && !npc.noTileCollide)
                {
                    ...
                    {
                        npc.localAI[2] = 1f;
                        Vector2 spawnPos = new Vector2(npc.position.X, npc.position.Y + npc.height - 2);
                        ...
                        if ((Main.player[npc.target].Center - Main.npc[(int)npc.ai[0]].Center).Length() < 300f && Main.rand.NextBool(3))
                        {
                ...
                else if ((int)npc.ai[2] == 2)
                {
                    ...
                    npc.noTileCollide = true;
                    npc.velocity = new Vector2(8f, 0f).RotatedBy((Main.player[npc.target].Center - npc.Center).ToRotation());
                ...
                else if ((int)npc.ai[2] == PHASE_PEARL)
                {
                    if (npc.ai[3] != -1)
                    {
                        int pearlType = ModContent.ProjectileType<CrabsonPearl>();
                        npc.ai[3]++;
                        if (npc.ai[3] < 30f)
                        {
                            _showPearl = true;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            {
                _mouthRotation = Utils.AngleLerp(_mouthRotation, _mouthGotoRotation, _mouthGotoLerp);
                if ((_mouthRotation - _mouthGotoRotation).Abs() < 0.0157f)
                {
                    _mouthRotation = _mouthGotoRotation;
                }
            }
            var texture = AQTextureAssets.GetNPC(npc.type);
            ...
            if (_mouthRotation != 0f)
            {
                if (_showPearl)
                {
                    var pearlTexture = AQTextureAssets.GetProjectile(ModContent.ProjectileType<CrabsonPearl>());
        ...
        bool NPCIMethods.idecidetofallthroughplatforms.decide()
        {

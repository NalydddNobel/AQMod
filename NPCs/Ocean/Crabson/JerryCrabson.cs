...
namespace AQMod.NPCs.Ocean.Crabson
{
    ...
    public class JerryCrabson : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (npc.ai[2] == 0)
            {
                ...
                if (Math.Abs(distanceX) > 12f)
                {
                    ...
                    npc.velocity.X = MathHelper.Lerp(npc.velocity.X, (Main.expertMode ? 10f : 4f) * dir, 0.01f);
                }
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    if (!Collision.WetCollision(npc.position, npc.width, npc.height))
                    {
                        if (npc.ai[3] > 300)
                        {
                            if (npc.ai[3] % 30 == 0f)
                            {
                                Projectile.NewProjectile(new Vector2(npc.position.X + npc.width / 2f, npc.position.Y), new Vector2(0f + npc.velocity.X * 0.25f, -6f), ModContent.ProjectileType<CrabBubble>(), 10, 1f, Main.myPlayer);
        ...
        {
            var solidPosition = npc.position + new Vector2(0, -npc.height / 2f);
        ...
        public override void NPCLoot()
        {
            ...
            if (Main.hardMode || !NPC.downedBoss1 && !NPC.downedBoss2 && !NPC.downedBoss3 && !NPC.downedSlimeKing && !NPC.downedQueenBee)
                Item.NewItem(rect, ModContent.ItemType<Crabsol>());

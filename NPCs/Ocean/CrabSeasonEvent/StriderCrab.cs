...
using AQMod.Items.Armor.Strider;
...
{
    public sealed class StriderCrab : ModNPC, NPCIMethods.idecidetofallthroughplatforms
    {
        ...
        public override void AI()
        {
            ...
            if (hoverTileY != -1)
            {
                ...
                {
                    npc.ai[2]++;
                    if (Main.netMode != NetmodeID.MultiplayerClient && npc.ai[2] <= 90f)
                    {
                        if (npc.ai[2] % 30 == 0f)
                        {
                            int damage = 10;

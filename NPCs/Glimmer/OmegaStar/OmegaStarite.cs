...
namespace AQMod.NPCs.Glimmer.OmegaStar
{
    [AutoloadBossHead()]
    public class OmegaStarite : ModNPC
    {
        ...
        public class OmegaStariteOrb
        {
            ...
            public OmegaStariteOrb(Vector3 position, float scale = 1f, float radius = OmegaStarite.CIRCUMFERENCE, float rotation = 0f, float maxRotation = MathHelper.TwoPi / INNER_RING)
            {
        ...
        public void Init()
        {
            ...
            Projectile.NewProjectile(npc.Center, Vector2.Zero, ModContent.ProjectileType<OrbCollider>(), damage, 1f, Main.myPlayer, npc.whoAmI);
        }
                ...
                case PHASE_OMEGA_LASER:
                {
                    ...
                    else
                    {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                if (npc.localAI[1] == 0f)
                                {
                                    if (PlrCheck())
                                    {
                                        ...
                                        CameraManager.ScreenShakeYMagnitude = 8f;
                                        CameraManager.ScreenShakeYLerp = 0.02f;
                                        CameraManager.ScreenShakeYOffsetTime = 2;
                        ...
                        else
                        {
                            ...
                            Dust.NewDust(dustPos, width, height, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, AQNPC.StariteProjectileColor, 2f);
                            Dust.NewDust(dustPos, width, height, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, AQNPC.StariteProjectileColor, 2f);
                            Dust.NewDust(dustPos, width, height, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, AQNPC.StariteProjectileColor, 2f);
                        }
                    }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            if ((int)npc.ai[0] == -1)
            {
                ...
                CameraManager.ScreenShakeYMagnitude = range * 0.5f;
                CameraManager.ScreenShakeYLerp = 0.01f;
                CameraManager.ScreenShakeYOffsetTime = Math.Max(6 - (int)(range * 0.5), 1);
            }
            sortedOmegites.Sort((o, o2) => -(o.position.Z + o.drawOffset.Z).CompareTo(o2.position.Z + o2.drawOffset.Z));
            var omegiteTexture = SpriteUtils.Textures.Extras[ExtraID.Omegite];
            ...
            List<DrawMethod> drawOmegite = new List<DrawMethod>();
            ...
            Vector2 offset = new Vector2(npc.width / 2f, npc.height / 2f);
                ...
                if (Trailshader.ShouldDrawVertexTrails())
                {
                    ...
                    for (int i = 0; i < NPCID.Sets.TrailCacheLength[npc.type]; i++)
                    {
                        if (npc.oldPos[i] == new Vector2(0f, 0f))
                            break;
                        trueOldPos.Add(CameraManager.UpsideDown(npc.oldPos[i] + offset - Main.screenPosition));
                    }
                    if (trueOldPos.Count > 1)
                    {
                        const float radius = CIRCUMFERENCE / 2f;
                        var trailClr = AQNPC.StariteDisco ? Main.DiscoColor : new Color(35, 85, 255, 120);

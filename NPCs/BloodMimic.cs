...
namespace AQMod.NPCs
{
    public class BloodMimic : AIMimic
    {
        ...
        public override int SpawnNPC(int tileX, int tileY)
        {
            ...
            {
                Rectangle rectangle = new Rectangle(lastX - SPAWN_RECT_SIZE_HALF, lastY - SPAWN_RECT_SIZE_HALF, SPAWN_RECT_SIZE, SPAWN_RECT_SIZE);
                for (int j = 0; j < 50; j++)
                {
        ...
        public override void NPCLoot()
        {
            ...
            {
                List<int> choices = new List<int> { ModContent.ItemType<Items.Accessories.Shields.TargeoftheBlodded>(), };
                for (int i = 0; i < 3; i++)
                {
                    randDrops();
                }

                        ...
                        if (NPC.ai[1] > 30 * 4 + 90f || !Main.expertMode && NPC.ai[1] >= 30 * 2 + 90f) {
                            ...
                            if (timer == 0) {
                                if (Main.netMode != NetmodeID.Server) {
                                    ...
                                    if (Main.netMode != NetmodeID.Server) {
                                        SoundEngine.PlaySound(AequusSounds.ThunderClap, NPC.Center);
                                    }
                                    int dustAmount = 50;
...
public class RedSpriteFriendly2 : ModNPC {
    ...
    public override void OnChatButtonClicked(bool firstButton, ref string shopName) {
        if (!firstButton) {
            NPC.KillEffects();
            NPC.active = false;
            NPC.NewNPCDirect(Terraria.Entity.GetSource_NaturalSpawn(), NPC.Center, ModContent.NPCType<RedSprite>());
        }

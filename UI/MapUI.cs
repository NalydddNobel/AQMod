...
namespace AQMod.Content
{
    public static class MapUI
    {
        ...
        internal static void RenderOnMap(ref string mouseText)
        {
            ...
            if (PlayerMapUpgrades.MapUpgradeVisible(upgrades.CosmicTelescope) && Glimmer.IsGlimmerEventCurrentlyActive())
            {
                var texture = ModContent.GetTexture(TexturePaths.MapUI + "ultimatesword");
                var frame = new Rectangle(0, 0, texture.Width, texture.Height);
                var drawPos = MapDrawPosition(new Vector2(Glimmer.tileX + 0.5f, Glimmer.tileY - 3f));
                ...
                for (int i = 0; i < Glimmer.Layers.Count; i++)
                {
                    var layer = Glimmer.Layers[i];
                    for (int j = 0; j < 2; j++)
                    {
                        int d = j == 1 ? -1 : 1;
                        var pos = new Vector2(Glimmer.tileX + 0.5f + layer.Distance * d, 46f);
            ...
            if (PlayerMapUpgrades.MapUpgradeVisible(upgrades.BlightedSoul))
            {
                ...
                {
                    if (Main.npc[i].active && !AQNPC.Sets.Instance.NoMapBlip.Contains(Main.npc[i].type) && Main.npc[i].GetBossHeadTextureIndex() == -1 && !Main.npc[i].townNPC)
                    {

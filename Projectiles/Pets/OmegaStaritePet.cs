...
namespace Aequus.Projectiles.Pets
{
    public class OmegaStaritePet : ModProjectile
    {
        ...
        private void RenderOrb(Vector3 orb, float scale, Rectangle frame)
        {
            ...
            var drawPosition = PerspectiveHelper.GetParralaxPosition(new Vector2(center.X + orb.X, center.Y + orb.Y), orb.Z * 0.0314f) - Main.screenPosition;
            int frameNumber = 1;
            if (scale <= 0.925f)
                frameNumber = 2;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                orbScales[i] = PerspectiveHelper.GetParralaxScale(Projectile.scale, orbs[i].Z * 0.157f);
                if (orbs[i].Z > 0f)
                {

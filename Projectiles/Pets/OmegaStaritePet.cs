    ...
    public class OmegaStaritePet : ModProjectile
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (int i = 0; i < orbsCount; i++)
            {
                ...
                orbScales[i] = AQUtils.Perspective.GetParralaxScale(projectile.scale, orbs[i].Z * 0.157f);
                if (orbs[i].Z > 0f)
                {
                    var drawPosition = AQUtils.Perspective.GetParralaxPosition(new Vector2(center.X + orbs[i].X, center.Y + orbs[i].Y), orbs[i].Z * 0.0314f) - Main.screenPosition;
                    int frameNumber = 1;

...
namespace Aequus.Projectiles.Melee {
    public class PhaseDiscProj : ValariProj {
        ...
        public override void AI() {
            ...
            if (Projectile.localAI[1] > 0.6f && Main.rand.NextBool(2)) {
                ...
                ParticleSystem.New<BloomParticle>(ParticleLayer.AboveNPCs)
                    ...
                    Projectile.rotation.ToRotationVector2() * Main.rand.NextFloat(2f, 6f) * Projectile.direction,
                    (c * Main.rand.NextFloat(1f, 3f)) with { A = 120 },
                    c * 0.2f,
                    Main.rand.NextFloat(0.5f, 2f) * Projectile.localAI[1],
            ...
            for (int i = 0; i < Main.maxProjectiles; i++) {
                if (Main.projectile[i].active && i != Projectile.whoAmI && Main.projectile[i].type == Type && Projectile.owner == Main.projectile[i].owner
                    ...
                    if ((int)Main.projectile[i].ai[1] != (int)Projectile.ai[1] && Projectile.Distance(Main.player[Projectile.owner].Center) > Main.projectile[i].Distance(Main.player[Projectile.owner].Center)) {
                        ...
                        SoundEngine.PlaySound(AequusSounds.swordPowerReady with { Volume = 0.5f }, Projectile.Center);
                        return;
                    }
                }
            }
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            Main.spriteBatch.Draw(AequusTextures.Bloom0, drawPosition,
                null, color * Projectile.Opacity * 0.6f, 0f, AequusTextures.Bloom0.Size()/2f, Projectile.scale * 0.7f, SpriteEffects.None, 0f);

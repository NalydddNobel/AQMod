    ...
    private void CheckPrims() {
        if (thunderPrim == null) {
            thunderPrim = new ForceCoordTrailRenderer(TrailTextures.Trail[1].Value, TrailRenderer.DefaultPass,
                (p) => new Vector2(4f), (p) => new Color(255, 180, 160, 40) * (float)Math.Pow(DrawOpacity, 2f), obeyReversedGravity: false, worldTrail: false) { coord1 = 1f, coord2 = 0f, };
        }
        if (thunderBloomPrim == null) {
            thunderBloomPrim = new ForceCoordTrailRenderer(TrailTextures.Trail[3].Value, TrailRenderer.DefaultPass,
                (p) => new Vector2(22f), (p) => lightningBloomColor * (float)Math.Pow(DrawOpacity, 2f), obeyReversedGravity: false, worldTrail: false) { coord1 = 1f, coord2 = 0f, };
        }
    }
...
public class SurgeRodHitSound : NetSound {
    protected override SoundStyle InitDefaultSoundStyle() {
        return AequusSounds.ElectricHit.Value with { Volume = 0.5f, PitchVariance = 0.2f };
    }
}
...
namespace Aequus.Projectiles.Magic
{
    public class GamestarProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            GamestarRenderer.Particles.Add(new SquareParticle(Projectile.Center, Main.rand.NextVector2Unit() * Main.rand.NextFloat(2.5f), Color.White, 8));

            for (int i = 0; i < Projectile.oldPos.Length; i++)
            {
                if (Projectile.oldPos[i] == Vector2.Zero || Main.rand.NextFloat(1f) < 0.33f)
                    continue;
                GamestarRenderer.Particles.Add(new SquareParticle(Projectile.oldPos[i] + new Vector2(Main.rand.NextFloat(Projectile.width), Main.rand.NextFloat(Projectile.height)), Main.rand.NextVector2Unit() * Main.rand.NextFloat(2.5f), Color.White, Main.rand.Next(1, 7)));
            }
        }
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            if (Main.netMode != NetmodeID.Server)
            {
                ...
                for (int i = 0; i < amt; i++)
                {
                    GamestarRenderer.Particles.Add(new SquareParticle(target.Center + new Vector2(Main.rand.NextFloat(-target.width, target.width), Main.rand.NextFloat(-target.height, target.height)),

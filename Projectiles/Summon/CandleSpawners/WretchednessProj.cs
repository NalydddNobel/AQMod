...
{
    public class WretchednessProj : BaseGhostSpawner
    {
        ...
        protected override void SpawnEffects()
        {
            ...
            for (int i = 0; i < 40; i++)
            {
                var d = Dust.NewDustDirect(r.TopLeft(), r.Width, r.Height, DustID.MagicMirror, Scale: Main.rand.NextFloat(0.6f, 1.5f));
                d.velocity *= 2f;
                d.velocity.Y -= 3f;
                d.noGravity = true;
                d.fadeIn = d.scale + 1f;

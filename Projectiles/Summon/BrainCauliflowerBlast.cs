    ...
    public sealed class BrainCauliflowerBlast : ModProjectile
    {
            ...
            if (!_didEffects)
            {
                ...
                for (int i = 0; i < 28; i++)
                {
                    ...
                    if (Main.rand.NextBool(4))
                    {
                        ...
                        AequusEffects.BehindProjs.Add(particle);
                        continue;
                    }

                    AequusEffects.BehindProjs.Add(new BloomParticle(position, velocity, 
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            if (Main.rand.NextBool(chance) && aequus.ghostSlots < aequus.ghostSlotsMax && target.lifeMax < 1000 && target.defense < 50 &&
                NecromancyDatabase.TryGet(target, out var info) && info.PowerNeeded <= 2.1f)
            {

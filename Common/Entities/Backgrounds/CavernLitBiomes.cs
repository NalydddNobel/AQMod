...
namespace Aequus.Common.Entities.Backgrounds;
...
public class CavernsLightEmitter : ModSystem {
    ...
    private static bool _echoCache;
    ...
    public override void PreUpdateEntities() {
        ...
        _echoCache = Main.ShouldShowInvisibleWalls();
    ...
    private static Vector3 GetWantedLight(Player player) {
        var ugBackground = LoaderManager.Get<UndergroundBackgroundStylesLoader>().Get(player.CurrentSceneEffect.undergroundBackground.value);
        if (ugBackground is ICavernLitUGBackground cavernLit) {
            return cavernLit.GetLightColor();
        }
    ...
    private static bool WallAllowsLight(Tile tile) {
        return Main.wallLight[tile.WallType] || (!_echoCache && (tile.IsWallInvisible || tile.WallType == WallID.EchoWall));
    }
...
internal interface ICavernLitUGBackground {

...
public sealed class DedicatedGlobalItem : GlobalItem {
    ...
    public static void DrawDedicatedItemName(DrawableTooltipLine line) {
        ...
        for (int i = 0; i < sparkleCount; i++) {
            Color sparkleColor = line.Color.HueAdd(Helper.Wave(i, -0.05f, 0.05f)) with { A = 0 } * 1.2f;
            ...
            Main.spriteBatch.Draw(
                texture,
                sparklePosition + new Vector2(Helper.Wave(timer * 11f, -4f, 3f), MathF.Pow(timerWrapped * 2f, 2f) * -12f + 12f),
                frame,
                sparkleColor * MathF.Pow(scale, 30f),
                0f,
                origin,
                scale * Helper.Wave(timer * 20f, 0.5f, 1.1f),
                spriteEffects,
                0f
            );
        }

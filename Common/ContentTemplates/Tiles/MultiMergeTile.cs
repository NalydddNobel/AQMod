...
namespace Aequus.Common.ContentTemplates.Tiles;
...
public abstract class MultiMergeTile : ModTile {
    ...
    public void AddMerge(int with) {
        ...
        if (Main.netMode != NetmodeID.Server) {
            Instance<LoadingSystem>().OnPostSetupContent += () => {
    ...
    public static void EnsureCacheLength() {
        ...
        if (_mergeCache.Length != TileLoader.TileCount) {
            ArrayTools.ResizeAndPopulate(ref _mergeCache, TileLoader.TileCount, Array.Empty<byte>);
        }
    }
    ...
    public override void PostDraw(int i, int j, SpriteBatch spriteBatch) {
        ...
        for (int k = 0; k < Merges.Count; k++) {
            ...
            if (frame.CanRender) {
                spriteBatch.Draw(_mergeTextures[Merges[k]].Value, new Vector2(i, j) * 16f - Main.screenPosition + Helper.TileDrawOffset, frame.GetFrame(new FastRandom(i + j).WithModifier((ulong)(i * j))), lighting, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
            }
        }
    }
}
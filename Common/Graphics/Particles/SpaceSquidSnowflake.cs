...
namespace AQMod.Common.Graphics.Particles
{
    public class SpaceSquidSnowflake : ParticleType
    {
        ...
        public SpaceSquidSnowflake(Vector2 position, Vector2 velocity = default(Vector2), Color color = default(Color), float scale = 1f)
        {
        ...
        {
            velocity.X = MathHelper.Lerp(velocity.X, Main.windSpeed, 0.01f) + (float)Math.Sin(Main.GlobalTime + position.Y / 32f) * 0.02f;
            if (velocity.Y < 4f)
            {
                velocity.Y += 0.05f;
            }

    ...
    internal class AQTests
    {
        ...
        public static class TestFabrikMethod3D
        {
            ...
            public static void Apply()
            {
                ...
                var texture = TextureCache.OmegaStariteOrb.GetValue();
                var textureOrigin = texture.Size() / 2f;
                Main.spriteBatch.Draw(texture, ThreeDimensionsEffect.GetParralaxPosition(new Vector2(origin.X, origin.Y) + Main.screenPosition, origin.Z * 0.01f) - Main.screenPosition, null, Color.Black, 0f, textureOrigin, ThreeDimensionsEffect.GetParralaxScale(1f, origin.Z * 0.1f), Microsoft.Xna.Framework.Graphics.SpriteEffects.None, 0f);
                Main.spriteBatch.Draw(texture, ThreeDimensionsEffect.GetParralaxPosition(new Vector2(target.X, target.Y) + Main.screenPosition, target.Z * 0.01f) - Main.screenPosition, null, Color.Black, 0f, textureOrigin, ThreeDimensionsEffect.GetParralaxScale(1f, target.Z * 0.1f), Microsoft.Xna.Framework.Graphics.SpriteEffects.None, 0f);
                ...
                for (int i = 0; i < segments.Length; i++)
                {
                    Main.spriteBatch.Draw(texture, ThreeDimensionsEffect.GetParralaxPosition(new Vector2(positions[i].X, positions[i].Y) + Main.screenPosition, positions[i].Z * 0.1f) - Main.screenPosition, null, AQUtils.MovingRainbow(7f - i * 0.5f), 0f, textureOrigin, ThreeDimensionsEffect.GetParralaxScale(1f, positions[i].Z * 0.1f), Microsoft.Xna.Framework.Graphics.SpriteEffects.None, 0f);
                }
                Main.LocalPlayer.mouseInterface = true;
            }
        }

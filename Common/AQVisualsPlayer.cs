...
{
    public class AQVisualsPlayer : ModPlayer
    {
        ...
        public ushort CursorDyeID { get; private set; } = 0;
        public string CursorDye { get; private set; } = "";

        public void SetCursorDye(int type)
        {
            if (type <= 0 || type > CursorDyeManager.CursorDyeCount)
            {
                CursorDyeID = 0;
                CursorDye = "";
            }
            ...
            {
                CursorDyeID = (ushort)type;
                var cursorDye = CursorDyeManager.CursorDyeFromID(type);
                CursorDye = AQStringCodes.EncodeName(cursorDye.Mod, cursorDye.Name);
            }
        }

        public int equippedWingsVisualOverride;
        ...
        public override void Load(TagCompound tag)
        {
            ...
            if (!string.IsNullOrEmpty(dyeKey) && AQStringCodes.DecodeName(dyeKey, out string cursorDyeMod, out string cursorDyeName))
            {
                ...
                SetCursorDye(CursorDyeManager.CursorDyeIDFromKey(cursorDyeMod, cursorDyeName));
            }
            else
            {
        ...
        {
            if (player.wings != equippedWingsVisualOverride)
                equippedWingsVisualOverride = 0;
        ...
        {
            int d = Dust.NewDust(spawnPos + new Vector2(0f, -6f), 6, 6, ModContent.DustType<Dusts.MonoDust>(), 0, 0, 0, cataEyeColor);
        ...
        public override void ModifyDrawInfo(ref PlayerDrawInfo drawInfo)
        {
            ...
            var drawPlayer = drawInfo.drawPlayer.GetModPlayer<AQVisualsPlayer>();

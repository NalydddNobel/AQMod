...
public abstract class UnifiedMount : ModMount, ILocalizedModType {
    public readonly ModItem MountItem;
    public readonly ModBuff MountBuff;

    public UnifiedMount() {
        MountItem = new InstancedMountItem(this);
        MountBuff = new InstancedMountBuff(this);
    }
...
internal class InstancedMountBuff : InstancedBuff {
    protected readonly UnifiedMount _parent;

    public InstancedMountBuff(UnifiedMount parent)
        : base(parent.Name, parent.NamespacePath() + $"/{parent.Name.Replace("Mount", "")}Buff") {
        _parent = parent;
    }
...
internal class InstancedMountItem(UnifiedMount parent, int itemRarity = ItemRarityID.Yellow, int value = Item.gold * 5, SoundStyle? SoundOverride = null)
    : InstancedModItem(parent.Name + "Item", parent.NamespacePath() + $"/{parent.Name.Replace("Mount", "")}Item") {
    [CloneByReference]

...
namespace AQMod.Common
{
    ...
    {
        public const byte SomeOtherInvasion = 0;
        public const byte GlimmerEventInvasion = 1;
        ...
        public static void Apply()
        {
            ...
            if (GlimmerEvent.CanShowInvasionProgress())
            {
                currentInvasionType = GlimmerEventInvasion;
                InvasionType = GlimmerEventInvasion;
            }
            //else if (CrabSeason.InActiveZone(Main.LocalPlayer))
            //{
            //    currentInvasionType = SomeOtherInvasion;
            //    InvasionType = SomeOtherInvasion;
            //}
            ...
            if (InvasionType != byte.MaxValue)
            {
                ...
                switch (InvasionType)
                {
                    case SomeOtherInvasion:
                    {
                        invasionProgress = 1 / (float)1;
                        uiStyle = 0;
                        texture = SpriteUtils.Textures.Extras[ExtraID.CrabSeasonIcon];
                        text = AQText.CrabSeason().Value;
                        color = new Color(10, 50, 140, 128);
                    }
                    break;
                    ...
                    case GlimmerEventInvasion:
                    {
                        ...
                        uiStyle = 0;
                        texture = SpriteUtils.Textures.Extras[ExtraID.GlimmerEventEventIcon];

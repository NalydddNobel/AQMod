...
public sealed class RenamingSystem : ModSystem {
    ...
    public static string GetColoredDecodedText(string input, bool pulse = false) {
        ...
        string result = "";
        foreach (var i in _decodedTextList) {
            result += i.Type != DecodeType.None ? ColorTagProvider.Get(Colors.AlphaDarken(DecodeColors[i.Type]), i.Output) : i.Output;
        }
    ...
    public override void Load() {
        On_WorldFile.SaveWorld_bool_bool += On_WorldFile_SaveWorld;
    }

    private void On_WorldFile_SaveWorld(On_WorldFile.orig_SaveWorld_bool_bool orig, bool useCloudSaving, bool resetTime) {

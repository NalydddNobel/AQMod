...
public class BackpackSlotsUI : NewUILayer {
    ...
    public static void AddBackpackWarningTip(Item item) {
        ...
        var tooltip = new SpecialAbilityTooltipInfo(HoveringBackpackSlotName, Color.Lerp(Color.SaddleBrown * 1.5f, Color.White, 0.75f), ModContent.ItemType<ScavengerBag>());
        ...
        if (tooltip.tooltipLines.Count > 0) {
            SpecialAbilityTooltips.Tooltips.Add(tooltip);
        }
    }

    public BackpackSlotsUI() : base("Backpack", AequusUI.InterfaceLayers.Inventory_28, InterfaceScaleType.UI) { }
}
...
public abstract class AGenStep : ModType, ILocalizedModType, IPostSetupContent {
    ...
    public double TotalWeight { get; private set; } = 1f;

    public static UnifiedRandom Random => WorldGen.genRand;

    protected virtual double Weight => 1f;
    ...
    public virtual void OnGenerationReset() { }

    public virtual GenPass CreateGenerationPass() {
        return new PassLegacy(Name, Apply, TotalWeight);
    }
    ...
    /// <summary>Safely sets the progress message.</summary>
    ...
    /// <summary>Safely sets the progress bar.</summary>
    protected static void SetProgress(GenerationProgress progress, double progressValue, double prevProgress = 0f, double wantedProgress = 1f) {
        if (progress != null) {

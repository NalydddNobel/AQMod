...
namespace Aequus.CrossMod.Common {
    /// <summary>
    ...
    public class ModCallManager : ILoadable {
        ...
        public static object HandleModCall(object[] args) {
            ...
            try {
                ...
                //            int itemId = Get<int>();
                //            if (Aequus.InfoLogs) {
                //                Aequus.Instance.Logger.Info($"{mod.Name}: Setting Sentry On-AI function for {Lang.GetItemNameValue(itemId)}");
                //            }
                //            var interaction = SentryAccessoriesDatabase.GetSoftSupportInteraction(itemId);
                //            if (TryGet<Action<Projectile, Item, Player>>(out var simpleFunc)) {
                //                interaction.OnAI += (info) => simpleFunc(info.Projectile, info.Accessory, info.DummyPlayer);
                //                return Success;
                //            }
                //            if (TryGet<Action<Projectile, GlobalProjectile, Item, Player>>(out var simpleFunc2)) {
                //                interaction.OnAI += (info) => simpleFunc2(info.Projectile, info.SentryAccessoriesGlobalProj, info.Accessory, info.DummyPlayer);
                //                return Success;
                //            }
                //        }
                //        return Failure;

                //    case "SentryAccessoryOnCreateProjectile":
                //    case "SentryAccessoryOnShoot": {
                //            int itemId = Get<int>();
                //            if (Aequus.InfoLogs) {
                //                Aequus.Instance.Logger.Info($"{mod.Name}: Setting Sentry On-Shoot function for {Lang.GetItemNameValue(itemId)}");
                //            }

                //            var interaction = SentryAccessoriesDatabase.GetSoftSupportInteraction(itemId);
                //            if (TryGet<Action<Projectile, Item, Player>>(out var simpleFunc)) {
                //                interaction.OnShoot += (source, proj, globalProj, info) => simpleFunc(proj, info.Accessory, info.DummyPlayer);
                //                return Success;
                //            }
                //            if (TryGet<Action<IEntitySource, Projectile, Item, Player>>(out var simpleFunc2)) {
                //                interaction.OnShoot += (source, proj, globalProj, info) => simpleFunc2(source, proj, info.Accessory, info.DummyPlayer);
                //                return Success;
                //            }
                //            if (TryGet<Action<Projectile, Projectile, Item, Player>>(out var simpleFunc3)) {
                //                interaction.OnShoot += (source, proj, globalProj, info) => simpleFunc3(proj, info.Projectile, info.Accessory, info.DummyPlayer);
                //                return Success;
                //            }
                //            if (TryGet<Action<IEntitySource, Projectile, Projectile, Item, Player>>(out var simpleFunc4)) {
                //                interaction.OnShoot += (source, proj, globalProj, info) => simpleFunc4(source, proj, info.Projectile, info.Accessory, info.DummyPlayer);
                //                return Success;
                //            }
                //        }
                //        return Failure;

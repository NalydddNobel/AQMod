...
public class OmegaStaritePet : ModPet {
    ...
    private Single innerRingRotation;
    private Single innerRingRoll;
    private Single innerRingPitch;
    ...
    public override void AI() {
        ...
        Single distance = (center - gotoPos).Length();
        ...
        if (distance < 2000f) {
            ...
            Int32 directionX = Math.Sign(gotoPos.X - Projectile.Center.X);
            if (Math.Abs(Projectile.velocity.X) < 6f || Math.Sign(Projectile.velocity.X) != directionX) {
            ...
            Int32 directionY = Math.Sign(gotoPos.Y - Projectile.Center.Y);
            if (Math.Abs(Projectile.velocity.Y) < 6f || Math.Sign(Projectile.velocity.Y) != directionY) {
                if (Math.Sign(Projectile.velocity.X) != directionX) {
                    Projectile.velocity.X *= 0.985f;
                }
    ...
    private void DrawGlowy(Texture2D texture, Vector2 drawCoordinates, Rectangle frame, Color color, Single scale = 1f) {
        var origin = frame.Size() / 2f;
        for (Single f = 0f; f < MathHelper.TwoPi; f += MathHelper.PiOver2) {
    ...
    private void RenderOrb(Vector3 orb, Single scale, Rectangle frame) {
    ...
    public override Boolean PreDraw(ref Color lightColor) {
        ...
        Single rotMult = MathHelper.TwoPi / 5f;
        var orbs = new Vector3[5];
        Single[] orbScales = new Single[5];
        for (Int32 i = 0; i < 5; i++) {
    ...
    internal override InstancedPetBuff CreatePetBuff() {
        return new(this, (p) => ref p.GetModPlayer<AequusPlayer>().petOmegaStarite, lightPet: true);
    }

    internal override InstancedPetItem CreatePetItem() {
        return new(this, itemRarity: ItemRarityID.Master, value: Item.sellPrice(gold: 5), alphaOverride: new Color(250, 250, 250, 150));
    }
}
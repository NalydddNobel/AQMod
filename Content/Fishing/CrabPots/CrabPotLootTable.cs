...
public class CrabPotLootTable : LoadedType {
    public readonly Dictionary<int, List<CrabPotCatchRule>> Table = [];
    ...
    public void Add(int waterStyle, CrabPotCatchRule rule) {
        (CollectionsMarshal.GetValueRefOrAddDefault(Table, waterStyle, out _) ??= []).Add(rule);
    }
}
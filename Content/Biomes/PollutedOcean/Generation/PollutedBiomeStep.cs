...
public sealed class PollutedBiomeStep : AGenStep {
    ...
    public PollutedOceanGenerationSideConfig generationSide;

    protected override double Weight => 200f;
    ...
    void SetGenerationParameters(PollutedOceanGenerationSideConfig side) {
    ...
    void GenerateTileArrays() {
        int length = TileLoader.TileCount;
        ReplaceableTile = ArrayTools.New(length, (i) => true);
        SafeTile = ArrayTools.New(length, (i) => true);
    ...
    private static void GenerateWallArrays() {
        ...
        ReplaceableWall = ArrayTools.New(length, (i) => true);

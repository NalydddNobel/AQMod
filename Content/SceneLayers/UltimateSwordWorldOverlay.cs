...
namespace AQMod.Content.SceneLayers
{
    public sealed class UltimateSwordWorldOverlay : SceneLayer
    {
        ...
        {
            if (!closeEnoughToDraw() || OmegaStariteSceneManager.Scene > 1)
                return;
            ...
            if (hitbox.Contains((int)trueMouseworld.X, (int)trueMouseworld.Y) && AQMod.glimmerEvent.IsActive)
            {
                ...
                if (OmegaStariteSceneManager.Scene == 0 && !Main.gameMenu && !Main.gamePaused && Main.LocalPlayer.IsInTileInteractionRange((int)trueMouseworld.X / 16, (int)trueMouseworld.Y / 16))
                {
        ...
        public static Vector2 swordPos()
        {
            ...
            float y = (AQMod.glimmerEvent.tileY * 16) - 80f + (bobbingSin() * 8f);
            return new Vector2(x, y);
        }

        private bool closeEnoughToDraw()
        {
            return AQMod.glimmerEvent.GetTileDistance(Main.LocalPlayer) < Main.screenWidth / 16;
        }

        public override void Update()
        {
            if (!AQMod.glimmerEvent.IsActive || OmegaStariteSceneManager.Scene > 1 || !closeEnoughToDraw())
                return;

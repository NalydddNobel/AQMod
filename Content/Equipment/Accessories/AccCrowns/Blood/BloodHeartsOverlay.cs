...
public class BloodHeartsOverlay : ModResourceOverlay {
    public static float AnimationTimer { get; set; }
    ...
    private float GetHeartFade(BloodCrownPlayer modPlayer, int totalHearts, int corruptedHearts, int resourceNumber) {
        return resourceNumber == totalHearts - corruptedHearts && modPlayer.bloodHeartRegen > 15
            ? 1f - (modPlayer.bloodHeartRegen - 15f) / 35f
            : 1f;
    }
    ...
    private void DrawFancyHeart(ResourceOverlayDrawContext context, Player player, BloodCrownPlayer modPlayer, int totalHearts, int corruptedHearts) {
        float heartFade = GetHeartFade(modPlayer, totalHearts, corruptedHearts, context.resourceNumber);

        Main.spriteBatch.Draw(AequusTextures.BloomStrong, context.position + new Vector2(0f, -2f), null,
            Color.Black * heartFade, 0f, AequusTextures.BloomStrong.Size() / 2, 0.4f, SpriteEffects.None, 0f);
    ...
    private void DrawClassicHeart(ResourceOverlayDrawContext context, Player player, BloodCrownPlayer modPlayer, int totalHearts, int corruptedHearts) {
        float heartFade = GetHeartFade(modPlayer, totalHearts, corruptedHearts, context.resourceNumber);
    ...
    private void DrawBarHeart(ResourceOverlayDrawContext context, Player player, BloodCrownPlayer modPlayer, int totalHearts, int corruptedHearts) {
        float heartFade = GetHeartFade(modPlayer, totalHearts, corruptedHearts, context.resourceNumber);
        ...
        if (heartFade < 1f) {
            Main.spriteBatch.Draw(heart, position, heartFrame,
                Color.Red with { A = 0 } * MathF.Sin(heartFade * MathHelper.Pi), 0f, origin, 1f, SpriteEffects.None, 0f);
            Main.spriteBatch.Draw(heart, position, cracksFrame,
                Color.White with { A = 0 } * MathF.Sin(heartFade * MathHelper.Pi), 0f, origin, 1f, SpriteEffects.None, 0f);
        }
    }
    ...
    public override void PostDrawResource(ResourceOverlayDrawContext context) {
        ...
        int totalHearts = player.HeartCount();
        int corruptedHearts = modPlayer.BloodHearts;
        switch (context.DisplaySet) {
            case FancyClassicPlayerResourcesDisplaySet:
                ...
                DrawFancyHeart(context, player, modPlayer, totalHearts, corruptedHearts);
                break;
            ...
            case ClassicPlayerResourcesDisplaySet:
                ...
                DrawClassicHeart(context, player, modPlayer, totalHearts, corruptedHearts);
                break;
            ...
            case HorizontalBarsPlayerResourcesDisplaySet:
                ...
                DrawBarHeart(context, player, modPlayer, totalHearts, corruptedHearts);
                break;
        }
    }
}

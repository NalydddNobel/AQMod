...
public class WeightedHorseshoe : ModItem, IUpdateItemDye {
    ...
    public override void UpdateAccessory(Player player, bool hideVisual) {
        ...
        AequusRemakePlayer.accWeightedHorseshoe = Item;
        ...
        if (fallSpeed > DamagingFallSpeedThreshold) {
            AequusRemakePlayer.visualAfterImages = true;
        }
    }

    private void UpdateFloorEffects(Player player, AequusPlayer AequusRemakePlayer) {
        ...
        if (floorTile.HasUnactuatedTile && (Main.tileSolid[floorTile.TileType] || Main.tileSolidTop[floorTile.TileType]) && Helper.IsFalling(AequusRemakePlayer.transitionVelocity, player.gravDir)) {
            float oldFallSpeed = Math.Abs(AequusRemakePlayer.transitionVelocity.Y);
    ...
    public void UpdateItemDye(Player player, bool isNotInVanitySlot, bool isSetToHidden, Item armorItem, Item dyeItem) {
        ...
        var AequusRemakePlayer = player.GetModPlayer<AequusPlayer>();
        AequusRemakePlayer.showHorseshoeAnvilRope = true;
        AequusRemakePlayer.cHorseshoeAnvil = dyeItem.dye;

        UpdateFloorEffects(player, AequusRemakePlayer);
    }
    ...
    internal static void OnPostUpdateEquips(Player player, AequusPlayer AequusRemakePlayer) {
        int visualProj = ModContent.ProjectileType<WeightedHorseshoeVisual>();
        if (AequusRemakePlayer.showHorseshoeAnvilRope && Main.myPlayer == player.whoAmI && player.ownedProjectileCounts[visualProj] < 1) {
            Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, player.velocity * 0.5f, visualProj, 0, 0f, player.whoAmI);
        }

        if (AequusRemakePlayer.accWeightedHorseshoe == null || player.velocity.Y < DamagingFallSpeedThreshold) {
            return;
        }

...
public class MeathookProj : ModProjectile {
            ...
            if (player.grapCount < 10) {
                ...
                if (distance < size * 6f) {
                    if (!_playedChainSound) {
                        SoundEngine.PlaySound(AequusSounds.MeathookNearEnemy with { Volume = 0.8f, Pitch = -0.1f, PitchVariance = 0.1f, }, Main.npc[ConnectedNPC].Center);
                        _playedChainSound = true;
                    }
                }

                // Set immunity frames (and set cooldown.)
                if (distance < size * 2f && player.immuneTime < Meathook.ImmuneTime && player.TryGetModPlayer(out TimerPlayer timer) && !timer.IsTimerActive(Meathook.Timer_Id)) {
                    player.immune = true;
                    player.immuneTime = Meathook.ImmuneTime;
                    timer.SetTimer(Meathook.Timer_Id, Meathook.ImmuneCD);
                }
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
        ...
        SoundEngine.PlaySound(AequusSounds.MeathookActivate, target.Center);
    }

...
public abstract class BrittleArmor(int numTiers, int maxHits, EquipType equipType) : ModTexturedType, ILocalizedModType {
    public readonly int MaxHits = maxHits;
    public readonly EquipType EquipType = equipType;

    internal readonly BrittleArmorItem[] Items = new BrittleArmorItem[numTiers];
...
internal sealed class BrittleArmorItem : InstancedModItem, IPickItemSlotMovementAction, IAddSpecialTooltips {
    [CloneByReference]
    ...
    public void OverrideItemMovementAction(ref int result, Item[] inventory, int context, int slot, Item checkItem) {
        if (context == ItemSlot.Context.PrefixItem) {
            result = IPickItemSlotMovementAction.RESULT_SIMPLE_SWAP;
        }
    }
    ...
    public override void SetDefaults() {
        Item.width = 16;
        Item.height = 16;
        Item.CloneShopValues(ItemID.DivingHelmet);
    ...
    void IAddSpecialTooltips.AddSpecialTooltips(List<SpecialAbilityTooltipInfo> tooltips) {
        var keyword = new SpecialAbilityTooltipInfo(Language.GetTextValue("Mods.Aequus.Items.Keywords.Brittle.DisplayName"), Color.Lerp(Color.Blue, Color.White, 0.75f), _parent.Items[^1].Type);

        if (HitsLeft > 0) {
            keyword.AddLine(Language.GetTextValue("Mods.Aequus.Items.Keywords.Brittle.Description", HitsLeft));
        }
        keyword.AddLine(Language.GetTextValue("Mods.Aequus.Items.CommonTooltips.GoblinTinkererRepair", HitsLeft));
    ...
    public override void PostDrawInInventory(SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale) {
        ...
        if (!InventoryUI.ContextsInv.Contains(AequusUI.CurrentItemSlot.Context) || percentage >= 1f) {
            return;
        }
...
internal sealed class BrittleEquipTexture(string texturePath) : EquipTexture, IEquipTextureDraw {
    ...
    public void Draw(ref PlayerDrawSet drawInfo, Vector2 position, Rectangle frame, Color color, float rotation, Vector2 origin, SpriteEffects effects, int shader) {
        Texture2D texture = (_texture ??= ModContent.Request<Texture2D>(texturePath)).Value;

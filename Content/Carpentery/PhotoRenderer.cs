...
namespace Aequus.Content.Carpentery
{
    public class PhotoRenderer : RequestableRenderTarget
    {
        ...
        public static List<PhotoData> RenderRequests { get; private set; }

        public override void Load(Mod mod)
        {
            base.Load(mod);
            RenderRequests = new List<PhotoData>();
        }
        ...
        {
            if (RenderRequests.Count == 0 || RenderRequests[0].tileMap == null || RenderRequests[0].Texture == null)
                return;
        ...
        protected override void DrawOntoTarget(GraphicsDevice device, SpriteBatch spriteBatch)
        {
            ...
            if (RenderRequests[0].tileMap == null || !IsTargetUsable(_target) || !IsTargetUsable(helperTarget))
            {
            ...
            var texture = RenderRequests[0].Texture;
        ...
        private static void DrawCapture(TileMapCache map, PhotoData clip)
        {
            ...
            area.X = (int)(clip.fracX * Main.maxTilesX);
            area.Y = (int)(clip.fracY * Main.maxTilesY);
            ...
            Main.dayTime = clip.gameState.isDayTime;
            Main.time = clip.gameState.time;

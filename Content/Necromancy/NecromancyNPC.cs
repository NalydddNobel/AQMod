...
namespace Aequus.Content.Necromancy
{
    public class NecromancyNPC : GlobalNPC, IAddRecipes
    {
        ...
        public override Color? GetAlpha(NPC npc, Color drawColor)
        {
            ...
            {
                var color = GhostRenderer.GetColorTarget(Main.player[zombieOwner], renderLayer).getDrawColor().UseA(100);
        ...
        public override void UpdateLifeRegen(NPC npc, ref int damage)
        {
            if (ghostDebuffDOT > 0)
            {
                ...
                {
                    multiplier += Main.player[zombieOwner].Aequus().zombieDebuffMultiplier;
                }
                npc.AddRegen(-(int)(ghostDebuffDOT * multiplier));
        ...
        public bool CanSpawnZombie(NPC npc, bool justChecking = true)
        {
            ...
            var aequus = Main.player[zombieOwner].Aequus();
            if (Main.player[zombieOwner].HasMinionAttackTargetNPC && Main.player[zombieOwner].MinionAttackTargetNPC == npc.whoAmI)
            {
                //Main.NewText(slotsConsumed, Color.Orange);
                //Main.NewText(aequus.ghostSlotsOld, Color.Yellow);
                //Main.NewText(aequus.ghostSlotsOld + slotsConsumed, Color.Cyan);
                //Main.NewText(aequus.ghostSlotsMax, Color.Lime);
                if (aequus.ghostSlotsOld + slotsConsumed > aequus.ghostSlotsMax)
                {
        ...
        public void SpawnZombie(NPC npc)
        {
            ...
            if (n < 200)
            {
                ...
                if (playSound)
                {
                    if (Main.netMode == NetmodeID.Server)
                    {
                        var p = Aequus.GetPacket(PacketType.ZombieConvertEffects);
                        p.Write(npc.position.X);
                        p.Write(npc.position.Y);
                        p.Write(npc.width);
                        p.Write(npc.height);
                        p.Write(zombieOwner);
                        p.Write(renderLayer);
        ...
        {
            var clr = GhostRenderer.GetColorTarget(Main.player[player], renderLayer).getDrawColor().UseA(25) * 0.8f;
            for (int i = 0; i < 50; i++)
            {
                var d = Dust.NewDustDirect(position, width, height, ModContent.DustType<GhostDrainDust>(), newColor: clr.HueAdd(Main.rand.NextFloat(-0.02f, 0.02f)).UseA(25), Scale: Main.rand.NextFloat(0.75f, 1.75f));
                d.customData = Main.player[player];
                d.velocity = Main.rand.NextVector2Unit() * Main.rand.NextFloat(8f);
            }
            SoundEngine.PlaySound(RecruitZombieSound, position);
        }

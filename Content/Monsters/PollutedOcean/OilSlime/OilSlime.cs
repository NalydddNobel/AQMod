...
public class OilSlime : ModNPC, IBodyItemContainer, IOilSlimeInheritedBurning {
    ...
    public override void SetStaticDefaults() {
        ...
        NPCID.Sets.SpecificDebuffImmunity[Type][BuffID.OnFire] = false;
        NPCID.Sets.SpecificDebuffImmunity[Type][BuffID.OnFire3] = false;
    ...
    public override void ModifyNPCLoot(NPCLoot npcLoot) {
        ...
        IItemDropRule[] BodyRules = [
#if COPPER_KEY
            // 1/15 chance to contain a Copper Key inside of its body.
            new CommonBodyDropRule(ModContent.ItemType<Items.Tools.Keys.CopperKey>(), Items.Tools.Keys.CopperKey.DropRate);
#endif
            ...
            new OneFromOptionsBodyDropRule(4, 1, [.. ItemSets.IsJunk])
        ];

        // Register chains in this funky way because of build constants.
        for (int i = 1; i < BodyRules.Length; i++) {
            BodyRules[i - 1].OnFailedRoll(BodyRules[i]);
        }

        // Register the first body rule, since it is the start of the chain.
        npcLoot.Add(BodyRules[0]);
    }
    #endregion

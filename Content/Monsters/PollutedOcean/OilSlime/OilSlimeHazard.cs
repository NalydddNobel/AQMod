using Aequus.Common.ContentTemplates.Projectiles;
using System;
...
public class OilSlimeHazard() : FloorHazard(new(HurtsPlayers: true, HurtsNPCs: true, HurtsProjectiles: true)) {
    ...
    public override void AI() {
        ...
        if (OnFire) {
            if (Projectile.soundDelay == 0) {
                SoundEngine.PlaySound(AequusSounds.Oil_Looping_OilSlime with { Volume = 0.25f, PitchVariance = 0.5f, MaxInstances = 20 }, Main.rand.NextVector2FromRectangle(Projectile.Hitbox));
    ...
    public override bool PreDraw(ref Color lightColor) {
        ...
        foreach (Point p in HazardPoints) {
            FastRandom random = new FastRandom(p.X * p.Y).WithModifier((ulong)(p.X + p.Y));

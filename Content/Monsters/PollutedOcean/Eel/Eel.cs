    ...
    void HeadAI(bool inWater, bool inGround) {
        ...
        switch (State) {
            default: {
                    ...
                    // Idle velocity.
                    Vector2 wantedVelocity = new Vector2(NPC.spriteDirection, Helper.Wave(Timer * IdleWaveTimerMultiplier, -IdleWaveMagnitude, IdleWaveMagnitude));
                    Timer++;
                    // Idle speed
                    ...
                    if (hasTarget) {
                        ...
                        if (canSeeTarget) {
                            ...
                            if (speed > maxSpeed / 6f) {
                                if (NPC.soundDelay <= 0) {
                                    SoundEngine.PlaySound(AequusSounds.MovingEel.Value with { Volume = 2f }, NPC.Center);
                                }
    ...
    void BubbleEffects() {
        ...
        if (velocity.Length() > 3f) {
            ...
            if (Main.rand.NextBool(bubbleChance)) {
                var bubble = Instance<UnderwaterBubbles>().New();

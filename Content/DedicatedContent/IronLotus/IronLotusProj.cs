...
public class IronLotusProj : LegacyHeldSlashingSwordProjectile {
    ...
    public override Single GetVisualOuter(Single progress, Single swingProgress) {
        return -170f + ((Single)Math.Sin(swingProgress * MathHelper.Pi) + 1f) * 80f;
    }

    public override Single GetScale(Single progress) {
        return base.GetScale(progress) + (Single)Math.Sin(progress * MathHelper.Pi) * 0.4f;
    }
    ...
    public override void UpdateSwing(Single progress, Single interpolatedSwingProgress) {
        if (AnimProgress > 0.4f && AnimProgress < 0.6f) {
            ...
            Single particleDistance = (swordHeight + 40f) * Projectile.scale;
            Single particleRandomRotation = 0.3f;
    ...
    public override Boolean PreDraw(ref Color lightColor) {
        ...
        Single size = texture.Size().Length();
        var effects = SpriteEffects.None;
        var origin = new Vector2(0f, texture.Height);
        Single rotation = Projectile.rotation + rotationOffset;
        ...
        Single wave = MathF.Pow(MathF.Sin(AnimProgress * MathHelper.Pi), 2f);
        ...
        Single swingProgress = SwingProgress(AnimProgress);
        if (swingProgress > 0.1f && swingProgress < 0.9f) {
            Single progress = (swingProgress - 0.1f) / 0.8f;
            Single intensity = (Single)Math.Sin(progress * MathHelper.Pi);

        ...
        public override void RandomUpdate(int i, int j)
        {
            for (int k = -1; k <= 1; k++)
            {
                for (int l = -1; l <= 1; l++)
                {
                    ...
                    if (Main.tile[i + k, j + l].active() && Main.tile[i + k, j + l].type == Type)
                    {
                        Main.tile[i + k, j + l].type = (ushort)ModContent.TileType<NobleMushroomsNew>();
                        int X = Main.tile[i + k, j + l].frameX / 36;
                        int Y = Main.tile[i + k, j + l].frameY / 38;
                        Main.tile[i + k, j + l].frameX = (short)(Y * 36);
                        if (Main.tile[i + k, j + l].frameX % 36 != 0)
                        {
                            Main.tile[i + k, j + l].frameX += 18;
                        }
                        if (Main.tile[i + k, j + l].frameY % 38 == 0)
                        {
                            Main.tile[i + k, j + l].frameY = 0;
                        }
                        else
                        {
                            Main.tile[i + k, j + l].frameY = 18;
                        }
                        RandomUpdate(i + k, j + l);
                    }
                }
            }
        }

...
namespace Aequus.Tiles.Moss
{
    public class RadonMossTile : ModTile
    {
        ...
        {
            if (WorldGen.InWorld(i, j, 30) && Aequus.GameWorldActive && Main.rand.NextBool(3))
            {
                ...
                if (!solidTop)
                {
                    ...
                    EffectsSystem.ParticlesAboveDust.Add(new BloomParticle(loc, new Vector2(0f, Main.rand.NextFloat(-max, 0f)), Color.Black, Color.Black * 0.33f, Main.rand.NextFloat(0.5f, 1.6f), 0.2f, 0f));
                }
                if (Main.rand.NextBool(40) && (!solidTop || !Main.tile[i, j + 1].IsFullySolid() || !Main.tile[i + 1, j].IsFullySolid() || !Main.tile[i - 1, j].IsFullySolid()))
                {
                    EffectsSystem.ParticlesAboveLiquids.Add(new FogParticle(new Vector2(i * 16f + Main.rand.NextFloat(16f), j * 16f + Main.rand.NextFloat(16f)), Main.rand.NextVector2Unit() * Main.rand.NextFloat(0.1f, 0.25f), Color.Black, 0.005f, Main.rand.NextFloat(MathHelper.TwoPi)));
                }
            }
            return true;
        }

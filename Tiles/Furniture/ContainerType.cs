    ...
    public abstract class ContainerType : ModTile
    {
		...
		public override bool NewRightClick(int i, int j)
		{
			...
			if (Main.netMode == NetmodeID.MultiplayerClient && !isLocked)
			{
				...
				{
					NetMessage.SendData(MessageID.RequestChestOpen, -1, -1, null, x, (float)y, 0f, 0f, 0, 0, 0);
					Main.stackSplit = 600;
				}
			}
			else
			{
				if (isLocked)
				{
					if (ManageLockedChest(player, i, j, x, y) && Chest.Unlock(x, y))
					{
						...
						{
							NetMessage.SendData(MessageID.Unlock, -1, -1, null, player.whoAmI, 1f, (float)x, (float)y);
						}
					}
				}
				else

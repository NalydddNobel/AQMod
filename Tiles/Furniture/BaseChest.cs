...
namespace Aequus.Tiles.Furniture {
    public abstract class BaseChest<T> : BaseFurniture.Furniture where T : ModItem {
        ...
        public override LocalizedText DefaultContainerName(int frameX, int frameY) {
            return TextHelper.GetItemName<T>();
        }
        ...
        public virtual int HoverItem(int i, int j, int left, int top) {
            return Chest.IsLocked(left, top) ? ItemID.GoldCoin : ItemDrop;
        }
        ...
        protected void DrawBasicGlowmask(int i, int j, SpriteBatch spriteBatch, Texture2D texture, Color color) {
            ...
            try {
                ...
                spriteBatch.Draw(
                    texture,
                    new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + Helper.TileDrawOffset,
                    new Rectangle(tile.TileFrameX, 38 * (chest == -1 ? 0 : Main.chest[chest].frame) + tile.TileFrameY, 16, 16),
                    color,
                    0f,
                    Vector2.Zero,
                    1f,

...
namespace Aequus.Tiles.Herbs.Mistral;
public class MistralTile : HerbTileBase, ITileDrawSystem {
    ...
    void DrawAllPinwheels(SpriteBatch sb) {
        foreach (Point p in this.GetDrawPoints()) {
            Tile tile = Main.tile[p];
            if (tile.TileFrameX < FrameWidth * 2) {
                return;
            }
            var groundPosition = new Vector2(p.X * 16f + 8f, p.Y * 16f + 16f).Floor();
            var pinwheel = PaintsRenderer.TryGetPaintedTexture(p.X, p.Y, AequusTextures.MistralTile_Pinwheel.FullPath);
            sb.Draw(pinwheel, groundPosition - Main.screenPosition - new Vector2(0f, 20f), null, Lighting.GetColor(p), Main.tileFrame[Type] / (float)TurnFrames, pinwheel.Size() / 2f, 1f, SpriteEffects.None, 0f);
        }
    }

    void IDrawSystem.Activate() {
        DrawLayers.Instance.PostDrawMasterRelics += DrawAllPinwheels;
    }

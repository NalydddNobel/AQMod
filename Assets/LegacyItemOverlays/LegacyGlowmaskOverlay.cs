    ...
    public class LegacyGlowmaskOverlay : ItemOverlayData
    {
        ...
        public override void DrawHeld(Player player, AQPlayer aQPlayer, Item item, PlayerDrawInfo info)
        {
            ...
            if (item.useStyle == ItemUseStyleID.HoldingOut)
            {
                if (Item.staff[item.type])
                {
                    ...
                    if (ShouldDraw(true, item))
                        Main.playerDrawData.Add(new DrawData(texture, drawCoordinates3, drawFrame3, GetDrawColor(), drawRotation3, origin3, item.scale, info.spriteEffects, 0) { shader = this.Shader });
                    return;
                }
                ...
                if (ShouldDraw(true, item))
                    Main.playerDrawData.Add(new DrawData(texture, drawCoordinates2, drawFrame2, GetDrawColor(), drawRotation2, origin2, item.scale, spriteEffects, 0) { shader = this.Shader });
                return;
            }
            if (player.gravDir == -1f)
            {
                ...
                if (ShouldDraw(true, item))
                    Main.playerDrawData.Add(new DrawData(texture, drawCoordinates2, drawFrame2, GetDrawColor(), drawRotation2, origin2, item.scale, info.spriteEffects, 0) { shader = this.Shader });
                return;
            }

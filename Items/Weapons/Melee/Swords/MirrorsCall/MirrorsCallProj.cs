...
namespace Aequus.Items.Weapons.Melee.Swords.MirrorsCall;
public class MirrorsCallProj : HeldSwordProjectile {
    public override string Texture => AequusTextures.MirrorsCall.FullPath;
    ...
    protected override void UpdateSword(Player player, AequusPlayer aequus, float progress) {
        ...
        if (Projectile.localAI[1] == 0 && swingTime * 2 < swingTimeMax) {
            ...
            SoundEngine.PlaySound(AequusSounds.SwordSwipe.Value with { Pitch = 0.7f, PitchVariance = 0.3f, MaxInstances = 20, Volume = 0.4f }, Projectile.Center);
        }
    }

...
namespace AQMod.Items.Weapons.Melee.Spears
{
        ...
        public override void AI()
        {
            ...
            if (projOwner.itemAnimation < projOwner.itemAnimationMax / 5 * 3)
            {
                if (projectile.ai[1] == 0f)
                {
                    ...
                    for (int i = 0; i < 30; i++)
                    {
                        int d = Dust.NewDust(center + new Vector2((float)Math.Sin(i) * 10f, 0f).RotatedBy(rotation), 1, 1, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, HorizonSkies.BlueMonoDustColor);
                        Main.dust[d].velocity += projectile.velocity;
                        Main.dust[d].noGravity = true;
                        d = Dust.NewDust(center + new Vector2((float)Math.Cos(i) * 10f, 0f).RotatedBy(rotation), 1, 1, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, HorizonSkies.OrangeMonoDustColor);
                        Main.dust[d].velocity += projectile.velocity;
                        Main.dust[d].noGravity = true;
                    }
    ...
    public class HorizonSkiesBolt : ModProjectile
    {
        ...
        private void Hit(Vector2 position)
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                int d = Dust.NewDust(realPos, Main.projectile[p].width, Main.projectile[p].height, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, HorizonSkies.BlueMonoDustColor);
                Main.dust[d].velocity = Vector2.Normalize(Main.dust[d].position - position) * 10f;
                Main.dust[d].noGravity = true;
                d = Dust.NewDust(realPos, Main.projectile[p].width, Main.projectile[p].height, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, HorizonSkies.OrangeMonoDustColor);
                Main.dust[d].velocity = Vector2.Normalize(Main.dust[d].position - position) * 10f;
                Main.dust[d].noGravity = true;
            }
        ...
        public override void AI()
        {
            ...
            Vector2 center = projectile.Center;
            int d = Dust.NewDust(center + new Vector2((float)Math.Sin(projectile.ai[0]) * 25f, 0f).RotatedBy(projectile.rotation + MathHelper.PiOver2), 1, 1, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, HorizonSkies.BlueMonoDustColor);
            Main.dust[d].velocity *= 0.01f;
            Main.dust[d].noGravity = true;
            d = Dust.NewDust(center + new Vector2((float)Math.Sin(projectile.ai[0] + MathHelper.Pi) * 25f, 0f).RotatedBy(projectile.rotation + MathHelper.PiOver2), 1, 1, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, HorizonSkies.OrangeMonoDustColor);
            Main.dust[d].velocity *= 0.01f;
            Main.dust[d].noGravity = true;
    ...
    public class HorizonSkiesExplosion : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            int d = Dust.NewDust(center + new Vector2((float)Math.Sin(projectile.ai[0]) * 25f, 0f).RotatedBy(projectile.rotation + MathHelper.PiOver2), 1, 1, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, HorizonSkies.BlueMonoDustColor);
            Main.dust[d].noGravity = true;
            d = Dust.NewDust(center + new Vector2((float)Math.Cos(projectile.ai[0]) * 25f, 0f).RotatedBy(projectile.rotation + MathHelper.PiOver2), 1, 1, ModContent.DustType<Dusts.MonoDust>(), 0f, 0f, 0, HorizonSkies.OrangeMonoDustColor);
            Main.dust[d].noGravity = true;
        }

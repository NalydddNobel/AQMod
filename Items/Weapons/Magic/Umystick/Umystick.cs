...
namespace Aequus.Items.Weapons.Magic.Umystick;
    ...
    private void HandleHoldStyleFixed(Player player) {
        ...
        if (player.gravDir == 1) {
            if (aequus.itemCooldown == 0) {
                if (Main.myPlayer == player.whoAmI && maxSpeed && player.CanUseRightClickFeatures()) {
    ...
    public override void ModifyTooltips(List<TooltipLine> tooltips) {
        foreach (var t in tooltips) {
            if (t.Name.StartsWith("Tooltip")) {
                t.Text = t.Text.FormatWith(new { PlayerName = Main.LocalPlayer.name, });
            }
        }
    }

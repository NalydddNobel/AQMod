...
namespace Aequus.Items.Weapons.Necromancy.Sceptres.Revenant {
    public class RevenantSceptreProj : ModProjectile {
        ...
        public override void AI() {
            if ((int)Projectile.ai[0] == 1) {
                ...
                var d = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<RevenantParticle>(), Projectile.velocity.X, Projectile.velocity.Y, Scale: Main.rand.NextFloat(1.25f, 0.75f));
                d.velocity *= 0.6f;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
            ...
            Projectile.netUpdate = true;
            Projectile.NewProjectile(Projectile.GetSource_OnHit(target), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<RevenantSceptreProjOnHit>(), 0, 0f, Projectile.owner);
        }

        public override void Kill(int timeLeft) {
            if (Projectile.ai[0] <= 0f) {
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<RevenantSceptreProjOnHit>(), 0, 0f, Projectile.owner);
            }
        }

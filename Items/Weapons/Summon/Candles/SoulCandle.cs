...
namespace Aequus.Items.Weapons.Summon.Candles
{
    public abstract class SoulCandle : ModItem
    {
        ...
        protected void DefaultToCandle(int limit, int souls, int npc)
        {
            ...
            Item.DamageType = Aequus.NecromancyDamage; // Invisible damage type which should hopefully trick the game into believing it's some sort of summoner related item
            soulLimit = limit;
            useSouls = souls;
            defSoulLimit = limit;
            defUseSouls = souls;

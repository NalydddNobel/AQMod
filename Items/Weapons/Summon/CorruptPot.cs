...
namespace AQMod.Items.Weapons.Summon
{
    public class CorruptPot : ModItem
    {
        ...
        public override void SetDefaults()
        {
            ...
            item.shoot = ModContent.ProjectileType<ChomperHead>();
            item.buffType = ModContent.BuffType<CorruptPotBuff>();
            item.autoReuse = true;
        }
    ...
    public class ChomperHead : ModProjectile
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void AI()
        {
            ...
            if (target == -1)
            {
                ...
                for (int i = 0; i < Main.maxProjectiles; i++)
                {
                    ...
                    else if (Main.projectile[i].active && Main.projectile[i].type == projectile.type && Main.projectile[i].owner == projectile.owner && Main.projectile[i].ai[1] < 120)
                    {
                        if (leaderIndex == -1)
                        {
                            leaderIndex = i;
            ...
            else
            {
                ...
                if ((int)projectile.ai[1] < 120)
                {
                    ...
                    GotoPosition(gotoPosition, MathHelper.Clamp(l / 1000f, 0.1f, lerpAmount));
    ...
    public class CorruptPotBuff : ModBuff
    {
        public override void SetDefaults()
        {
        ...
        {
            var aQPlayer = player.GetModPlayer<AQPlayer>();
            if (player.ownedProjectileCounts[ModContent.ProjectileType<ChomperHead>()] > 0)
            {

...
{
    public class InactivePylonHealer : ModItem
    {
        ...
        public override bool? UseItem(Player player)
        {
            ...
            for (int i = -25; i < 25; i++)
            {
                for (int j = -25; j < 25; j++)
                {
                    ...
                    {
                        if (DroneSystem.FindOrAddDrone(x, y)?.AddDrone<HealerDroneType>() == true)
                        {
                            if (Main.myPlayer == player.whoAmI)
                            {
                                ...
                                var p = Projectile.NewProjectileDirect(player.GetSource_ItemUse(Item), spawnPosition,
                                    Vector2.Normalize(mousePosition - spawnPosition) * Item.shootSpeed, ModContent.ProjectileType<HealerDrone>(), 0, 0f, Main.myPlayer);

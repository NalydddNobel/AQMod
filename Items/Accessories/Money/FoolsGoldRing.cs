...
namespace Aequus.Items.Accessories.Money {
        ...
        public static void OnKillEffects(int x, int y, int width, int height) {
            if (Main.netMode != NetmodeID.Server) {
                ...
                for (int i = 0; i < amt; i++) {
                    ...
                    ParticleSystem.New<ShinyFlashParticle>(ParticleLayer.AboveDust).Setup(Helper.NextFromRect(Main.rand, rect), Vector2.Zero, Color.Yellow.UseA(0), Color.White * 0.33f, Main.rand.NextFloat(0.5f, 1f) * intensity, 0.2f, 0f);
                }
                ...
                for (int i = 0; i < amt * 3; i++) {
                    ...
                    d.velocity *= 0.5f;
                    d.velocity = Utils.SafeNormalize(d.position - center, Vector2.UnitY) * d.velocity.Length();
                }
            }
        }

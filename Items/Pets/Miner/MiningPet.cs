...
namespace Aequus.Items.Pets.Miner {
    public class MiningPet : ModProjectile {
        ...
        public override bool PreAI() {
            ...
            if (swingPick > 0) {
                ...
                else if (Projectile.velocity.X.Abs() > 0f) {
                    if (frame < 2)
                        frame = 2;

                    frameCounter += Projectile.velocity.X.Abs() * 0.7f;
                    if (frameCounter >= 6f) {
                        frameCounter = 0f;
                        frame++;
                        if (frame > 10)
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            if (swingPick > 0) {
                ...
                if (Projectile.spriteDirection == -1) {
                    origin.X = TextureAssets.Item[pickaxeItemID].Value.Width;
                }
                else {
                    rotation = -rotation;
                }

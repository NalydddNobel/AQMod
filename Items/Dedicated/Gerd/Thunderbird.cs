...
namespace AQMod.Items.Dedicated.Gerd
{
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            ...
            if (aQPlr.thunderbirdLightningTimer <= 0)
            {
                ...
                if (validNPCs.Count > 0)
                {
                    ...
                    Vector2 spawnPosition = new Vector2(Main.rand.Next((int)npc.position.X - 20, (int)npc.position.X + npc.width + 20), (int)player.position.Y - 600);
                    if (Collision.CanHitLine(npc.position, npc.width, npc.height, spawnPosition, 2, 2))
                    {
                        ...
                        for (int j = 0; j < segments; j++)
                        {
                            ...
                            Vector2 toPos = Vector2.Normalize(lastPos - pos);

...
public abstract class EnergyItemBase : ModItem {
    ...
    public override void Update(ref float gravity, ref float maxFallSpeed) {
        if (Item.timeSinceItemSpawned % 16 == 0) {
            ParticleSystem.New<EnergyParticle>(ParticleLayer.AboveItems).Setup(Item.position + new Vector2(Main.rand.Next(Item.width), Main.rand.NextFloat(8f, Item.height * 0.66f)), -Vector2.UnitY * Main.rand.NextFloat(0.1f, 0.22f), new Color(LightColor * 2f) with { A = 0 }, Main.rand.NextFloat(0.8f, 1.1f));
        }
        if (Item.timeSinceItemSpawned % 28 == 0) {
            ParticleSystem.New<EnergyParticle>(ParticleLayer.AboveNPCs).Setup(Item.position + new Vector2(Main.rand.Next(Item.width), Main.rand.NextFloat(8f, Item.height * 0.66f)), -Vector2.UnitY * Main.rand.NextFloat(0.1f, 0.22f), new Color(LightColor * 0.75f) with { A = 0 }, Main.rand.NextFloat(0.8f, 1.1f));
        }

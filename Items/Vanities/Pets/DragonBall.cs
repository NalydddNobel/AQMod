...
namespace AQMod.Items.Vanities.Pets
{
    public class DragonBall : ModItem
    {
        public override void SetDefaults()
        {
            ...
            item.expert = true;
            item.shoot = ModContent.ProjectileType<MicroStaritePet>();
            item.buffType = ModContent.BuffType<MicroStariteBuff>();
        }
    ...
    public class MicroStariteBuff : ModBuff
    {
        public override void SetDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            player.GetModPlayer<AQPlayer>().microStarite = true;
            bool petProjectileNotSpawned = player.ownedProjectileCounts[ModContent.ProjectileType<MicroStaritePet>()] <= 0;
            if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
                Projectile.NewProjectile(player.position.X + player.width / 2, player.position.Y + player.height / 2, 0f, 0f, ModContent.ProjectileType<MicroStaritePet>(), 0, 0f, player.whoAmI, 0f, 0f);
        }
    }

    public class MicroStaritePet : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (distance < (player.width + projectile.width) * 2f)
            {
                ...
                projectile.velocity += player.velocity * 0.05f;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (int i = 0; i < orbsCount; i++)
            {
                ...
                orbScales[i] = Parralax.GetParralaxScale(projectile.scale, orbs[i].Z * 0.157f);
                if (orbs[i].Z > 0f)
                {
                    var drawPosition = Parralax.GetParralaxPosition(new Vector2(center.X + orbs[i].X, center.Y + orbs[i].Y), orbs[i].Z * 0.0314f) - Main.screenPosition;
                    int frameNumber = 1;

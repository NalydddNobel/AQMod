...
namespace AQMod.Items.Materials.Energies
{
    public class UltimateEnergy : ModItem
    {
        ...
        {
            AQItem.DrawEnergyItemInv(spriteBatch, AQMod.Coloring.UltimateGrad, item, position, origin, scale);
            return false;
        }

        public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
        {
            AQItem.DrawEnergyItemWorld(spriteBatch, AQMod.Coloring.UltimateGrad, item, rotation, scale);
            return false;
        }

        public override void Update(ref float gravity, ref float maxFallSpeed)
        {
            AQItem.UpdateEnergyItem(item, AQMod.Coloring.UltimateGrad.GetColor(Main.GlobalTime), new Vector3(0.3f, 0.3f, 0.8f));
        }

...
namespace Aequus
{
    ...
    public partial class AequusTile
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ProcExtraOres(int i, int j, int type, Player player, AequusPlayer aequus)
        {
            if (aequus.extraOresChance.RollLuck(player) && TileIDToItemID.TryGetValue(type, out int extraOre))
            {
                Item.NewItem(
                    new EntitySource_TileBreak(i, j, "Aequus: Extra Ore"),

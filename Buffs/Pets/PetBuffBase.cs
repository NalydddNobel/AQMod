    ...
    public abstract class PetBuffBase : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[PetProj] <= 0 && player.whoAmI == Main.myPlayer)
                Projectile.NewProjectile(player.GetProjectileSource_Buff(buffIndex), player.position.X + player.width / 2f, player.position.Y + player.height / 2f, 0f, 0f, PetProj, 0, 0f, player.whoAmI, 0f, 0f);
        }
    }
}

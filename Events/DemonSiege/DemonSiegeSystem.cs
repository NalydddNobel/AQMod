...
namespace Aequus.Events.DemonSiege
{
    public class DemonSiegeSystem : ModSystem
    {
        ...
        public override void PostUpdateNPCs()
        {
            ...
            foreach (var remove in SacrificeRemovalQueue)
            {
                ActiveSacrifices.Remove(remove);
                if (Main.netMode != NetmodeID.SinglePlayer)
                {
                    var p = Aequus.GetPacket(PacketType.RemoveDemonSiege);
                    p.Write((ushort)remove.X);
                    p.Write((ushort)remove.Y);
                    p.Send();
                }
            }
            SacrificeRemovalQueue.Clear();
        ...
        public override void PostDrawTiles()
        {
            ...
            {
                var auraTexture = AequusHelpers.GetTexture(this.GetNoNamePath(), "GoreNestAura");
                Main.spriteBatch.Begin_World(shader: false); ;
                try
                {
                    DrawDemonSiegeRanges(auraTexture);
                }
                catch
                {
        ...
        public static bool NewInvasion(int x, int y, Item sacrifice, int player = byte.MaxValue, bool checkIsValidSacrifice = true, bool allowAdding = true, bool allowAdding_IgnoreMax = false)
        {
            ...
            if (Main.netMode != NetmodeID.SinglePlayer)
            {
                SendSacrificeStart(s, (byte)player);
            }
        ...
        public static void ReceiveStartRequest(BinaryReader reader)
        {
            ...
            {
                SendSacrificeStart(s, player);
            }
        }

        public static void SendSacrificeStart(DemonSiegeSacrifice s, byte plr)
        {
            var p = Aequus.GetPacket(PacketType.StartDemonSiege);
            var bb = new BitsByte(s.unholyCoreUsed);
            p.Write((ushort)s.TileX);
            p.Write((ushort)s.TileY);
            p.Write(plr);
            p.Write(bb);
            ItemIO.Send(s.Items[0], p, writeStack: true, writeFavorite: false);

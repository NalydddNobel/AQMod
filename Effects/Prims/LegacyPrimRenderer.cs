...
namespace Aequus.Effects.Prims
{
    public class LegacyPrimRenderer : IPrimRenderer, ILoadable
    {
        ...
        public LegacyPrimRenderer()
        {
        }

        public LegacyPrimRenderer(Texture2D texture, string pass, Func<float, Vector2> getWidth, Func<float, Color> getColor, bool obeyReversedGravity = true, bool worldTrail = true, Vector2 drawOffset = default(Vector2))
        {
            Texture = texture;
            Pass = pass;
            GetWidth = getWidth;
